                ...
                <div class="header d-flex justify-content-between" style="padding-bottom: 0;">
                    ...
                    <div class="form-group col-2 mb-0">
                        ...
                        <select class="form-control" name="status" id="myselect">
                            ...
                            <option value="printmanifest">Print Manifest</option>
...
<!--                                            <img src="{{ asset('public/favicon.svg') }}" style="width:50px"
                                        ...
                                        <td> 
                                            ...
                                            @endif
                                                ...
                                                <a href="{{ URL::to('admin/tracking/' . $row->tracking_info) }}" target="_blank">{{ $couriers[$row->ship_courier_id]['name'] }} 
                                                    @if($row->shipping_courier_type == 'fa-plane') <span class="badge badge-info">Air</span>
                                                    @elseif($row->shipping_courier_type == 'fa-truck') <span class="badge badge-warning">Surface</span>
                                                    @elseif($row->shipping_courier_type == 'fa-motorcycle') <span class="badge badge-success">SDD</span>
                                                    @elseif($row->shipping_courier_type == 'fa-bicycle') <span class="badge badge-primary">NDD</span>
                                                    @elseif($row->shipping_courier_type == 'fa-bolt') <span class="badge badge-danger">Quick</span>
                                                    @endif
                                                </a>
                                        ...
                                        <td>
                                                ...
                                                <div class="d-flex justify-content-between align-items-center">
                                                    ...
                                                    <div class="order-status-text-{{ $row->id }}">
...
<script type="text/javascript">
    (function($) {
        ...
        // Auto-update tracking statuses in background on page load/refresh & every 30 seconds
        $(document).ready(function() {
            function checkTrackingUpdates() {
                ...
                if (visibleIds.length > 0) {
                    $.ajax({
                        url: "{{ route('get.track.update') }}",
                        type: "GET",
                        data: { ids: visibleIds },
                        success: function(response) {
                            if (response.success && response.updated && response.orders) {
                                response.orders.forEach(function(order) {
                                    $('.order-status-text-' + order.id).html(order.status_html);
                                });
                            }
                        }
                    });
            ...
            setInterval(checkTrackingUpdates, 30000);
        });
        ...
        $('select[name=status]').change(function() {
            ...
            if (action_type == 'download_all' || $('input[name^="id"]:checked').length > 0) {
                ...
                else if(action_type == 'printmanifest'){
                    let action_route = `{{ route('admin.order.printselectedmanifest') }}`;
                    $('#myForm').attr("action", action_route);
                }
